...
### 왜 서버리스 구조를 선택했나요?
이 서비스는 공지를 실시간으로 전달하는 것이 아니라, 여러 게시판의 새 글을 매일 빠짐없이 확인하는 것을 목표로 합니다. 공지들은 실시간성이 낮고 신청 기간도 길기 때문에, 정확한 실행 시각보다 정기적인 수집이 더 중요합니다. 따라서 실행이 일부 지연될 수 있는 GitHub Actions 크론도 요구사항을 충분히 충족합니다.

크롤링은 하루 두 번, 몇 초 동안만 실행되며 저장할 상태도 게시판별로 확인한 글 ID뿐입니다. 별도의 서버와 DB를 운영하는 대신, GitHub Actions로 작업을 실행하고 `state.json`을 Git에 커밋해 상태를 관리하는 단순한 구조를 선택했습니다. 이를 통해 필요한 기능은 충족하면서 인프라 비용과 운영 복잡도를 최소화했습니다.
