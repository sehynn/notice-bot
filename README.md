...
- 이 프로젝트의 크롤링 작업은 하루 두 번 실행되며, 전체 게시판을 확인하는 데 몇 초밖에 걸리지 않습니다. 또한 저장해야 하는 데이터도 게시판별로 이미 확인한 게시글 ID뿐입니다.

- 이 정도의 작업을 위해 별도의 VPS나 클라우드 인스턴스, 데이터베이스를 구축하고 관리하는 것은 요구사항에 비해 과도하다고 판단했습니다. 사용자별 상태나 복잡한 조회가 필요하지 않아, 게시글 ID를 저장하는 파일만으로도 새로운 공지를 판별할 수 있었습니다.

- 따라서 GitHub Actions의 크론으로 크롤링을 실행하고, 확인한 게시글 ID는 `state.json`에 저장하도록 구성했습니다. 작업이 끝나면 변경된 `state.json`을 저장소에 커밋해 다음 실행에서도 이전 상태를 이어서 사용합니다.

- 이를 통해 별도의 서버 비용 없이 운영할 수 있으며, 인프라와 데이터베이스 관리에 필요한 유지보수 부담도 최소화했습니다.
